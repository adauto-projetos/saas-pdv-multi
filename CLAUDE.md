...
Os padrões de cada área vivem na discovery da feature correspondente — leia `docs/features/[ID]/discovery.md` antes de implementar/revisar algo daquela área. A skill global `project-patterns` (padrões detalhados por área, carregados sob demanda) ainda não foi gerada: rodar `/add.xray` numa chore própria (operação generativa pesada) — não assumir que ela existe.
