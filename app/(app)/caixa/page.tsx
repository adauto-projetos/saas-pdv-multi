import { getOpenSessionAction } from "@/app/(app)/lucro/actions";
import { listProductCategoriesAction } from "@/app/(app)/products/categories/actions";
import { listProductsForCaixaAction } from "@/app/(app)/products/actions";
import { CaixaShell } from "@/components/caixa/CaixaShell";

export const dynamic = "force-dynamic";

export default async function CaixaPage() {
  const [productsResult, categoriesResult, sessionResult] = await Promise.all([
    listProductsForCaixaAction(),
    listProductCategoriesAction(),
    getOpenSessionAction(),
  ]);
  const products = productsResult.ok ? productsResult.data : [];
  // Chips do caixa (RF07): categorias do tenant por position; sem cache client —
  // mudanças refletem no próximo render RSC (revalidatePath das actions, RN07).
  const categories = categoriesResult.ok ? categoriesResult.data : [];
  // session=undefined → sem permissão "caixa" (action falhou); CaixaShell esconde o
  // botão do turno. session=null → permissão ok mas sem turno aberto.
  const session = sessionResult.ok ? sessionResult.data : undefined;
  return (
    <CaixaShell products={products} categories={categories} session={session} />
  );
}
